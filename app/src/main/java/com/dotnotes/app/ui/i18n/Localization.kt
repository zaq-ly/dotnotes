package com.dotnotes.app.ui.i18n

import androidx.compose.runtime.staticCompositionLocalOf

data class AppStrings(
    val appName: String = ".notes",
    val back: String,
    val save: String,
    val cancel: String,
    val delete: String,
    val markAsDone: String,
    val untitled: String,
    val edited: String,
    
    // Note List
    val noNotesYet: String,
    val pinned: String,
    val others: String,
    val newNote: String,
    val pin: String,
    val unpin: String,
    val searchNotes: String,
    val scheduledAt: String,
    
    // Note Editor
    val editNote: String,
    val noteTitleHint: String,
    val noteContentHint: String,
    val reminder: String,
    val priority: String,
    val notification: String,
    val alarm: String,
    val setReminder: String,
    val clearReminder: String,
    val selectDate: String,
    val selectTime: String,
    val alarmIn: String,
    val reminderIn: String,
    val lessThanOneMinute: String,
    val timeHours: String,
    val timeMinutes: String,
    val timeDays: String,
    val repeat: String,
    val repeatNone: String,
    val repeatDaily: String,
    val repeatWeekly: String,
    val repeatMonthly: String,
    val repeatYearly: String,
    val selectedCount: String,
    val selectAll: String,
    val deselectAll: String,
    val noteColor: String,
    val colorDefault: String,
    val colorBlue: String,
    val colorRed: String,
    val colorYellow: String,
    val colorGreen: String,
    val colorPurple: String,
    val colorRose: String,
    val colorOrange: String,
    val colorTeal: String,
    val checklist: String,
    val addChecklistItem: String,
    val checkedItemsCount: String,
    
    // Settings
    val settings: String,
    val language: String,
    val english: String,
    val indonesian: String,
    val theme: String,
    val themeSystem: String,
    val themeLight: String,
    val themeDark: String,
    val snoozeDuration: String,
    val reminderSound: String,
    val alarmSound: String,
    val defaultSound: String,
    val minutes: String,
    val exportBackup: String,
    val exportBackupDesc: String,
    val importBackup: String,
    val importBackupDesc: String,
    val exportSuccess: String,
    val exportFailed: String,
    val importSuccess: String,
    val importFailed: String,
    val checkForUpdates: String,
    val checkForUpdatesDesc: String,
    val alreadyLatest: String,
    val updateAvailable: String,
    val updateNow: String,
    val later: String,
    val downloadingUpdate: String,
    val about: String,
    
    // Alarm Screen
    val noteReminder: String,
    val snooze: String,
    val dismiss: String,

    // History & Reminders
    val history: String,
    val reminderHistory: String,
    val pendingTasks: String,
    val completedTasks: String,
    val noPendingTasks: String,
    val noCompletedTasks: String,
    val markDone: String,
    val reminderDoneRepeated: String,
    val reminderDoneOnce: String,
    val reminderDoneMovedToHistory: String,
    val stopRecurring: String,
    val stopRecurringPermanently: String,
    val completeThisSession: String,
    val completeThisSessionDesc: String,
    val stopRecurringDesc: String,
    val recurringStopped: String,
    val stopSchedule: String,
    val restore: String,
    val restoredToNotes: String,
    val completed: String,
    val overdue: String,

    // Cloud Backup & Auth
    val cloudBackup: String,
    val cloudBackupDesc: String,
    val googleAccount: String,
    val signInWithGoogle: String,
    val signOut: String,
    val syncNow: String,
    val syncing: String,
    val syncSuccess: String,
    val syncUpToDate: String,
    val syncFailed: String,
    val notLoggedIn: String,
    val supabaseNotConfigured: String,
    val lastSynced: String,
    val never: String,
    val accountAndSync: String,
    val preferences: String,
    val aboutAndUpdates: String,
    val newBadge: String,
    val googleSignInPrompt: String,
    val signingIn: String,
    val autoSyncActive: String,
    val autoArchive: String,
    val autoArchiveDesc: String,
    val alarmAutoArchiveNotice: String,
    val alarmKeepOnHomeNotice: String,
    val archive: String,
    val notesArchived: String,
    val locale: java.util.Locale = java.util.Locale.ENGLISH
)

val EnglishStrings = AppStrings(
    locale = java.util.Locale.ENGLISH,
    back = "Back",
    save = "Save",
    cancel = "Cancel",
    delete = "Delete",
    markAsDone = "Mark as Done",
    untitled = "Untitled",
    edited = "Edited",
    noNotesYet = "No notes yet",
    pinned = "Pinned",
    others = "Others",
    newNote = "New Note",
    pin = "Pin",
    unpin = "Unpin",
    searchNotes = "Search notes...",
    scheduledAt = "Reminder for %s",
    editNote = "Edit Note",
    noteTitleHint = "Title",
    noteContentHint = "Write note...",
    reminder = "Reminder",
    priority = "Priority",
    notification = "Reminder",
    alarm = "Alarm",
    setReminder = "Set date & time",
    clearReminder = "Clear",
    selectDate = "Select Date",
    selectTime = "Select Time",
    alarmIn = "Alarm in %s",
    reminderIn = "Reminder in %s",
    lessThanOneMinute = "< 1 min",
    timeHours = "hr",
    timeMinutes = "min",
    timeDays = "days",
    repeat = "Repeat",
    repeatNone = "Does not repeat",
    repeatDaily = "Every day",
    repeatWeekly = "Every week",
    repeatMonthly = "Every month",
    repeatYearly = "Every year",
    selectedCount = "%d selected",
    selectAll = "Select All",
    deselectAll = "Deselect All",
    noteColor = "Note Color",
    colorDefault = "Default",
    colorBlue = "Blue",
    colorRed = "Red",
    colorYellow = "Yellow",
    colorGreen = "Green",
    colorPurple = "Purple",
    colorRose = "Rose",
    colorOrange = "Orange",
    colorTeal = "Teal",
    checklist = "Checklist",
    addChecklistItem = "List item",
    checkedItemsCount = "%d checked items",
    settings = "Settings",
    language = "Language",
    english = "English",
    indonesian = "Bahasa Indonesia",
    theme = "Theme",
    themeSystem = "System",
    themeLight = "Light",
    themeDark = "Dark",
    snoozeDuration = "Snooze Duration",
    reminderSound = "Reminder Sound",
    alarmSound = "Alarm Sound",
    defaultSound = "Default",
    minutes = "minutes",
    exportBackup = "Export Backup",
    exportBackupDesc = "Save notes to a JSON file",
    importBackup = "Import Backup",
    importBackupDesc = "Restore notes from a JSON file",
    exportSuccess = "Backup exported successfully",
    exportFailed = "Failed to export backup",
    importSuccess = "Successfully imported %d notes",
    importFailed = "Failed to import backup file",
    checkForUpdates = "Check for Updates",
    checkForUpdatesDesc = "Check for newer release on GitHub",
    alreadyLatest = "App is already up to date",
    updateAvailable = "Update Available",
    updateNow = "Update Now",
    later = "Later",
    downloadingUpdate = "Downloading update...",
    about = "About",
    noteReminder = "NOTE REMINDER",
    snooze = "Snooze",
    dismiss = "Dismiss",
    history = "History",
    reminderHistory = "History & Archive",
    pendingTasks = "Unfinished",
    completedTasks = "Completed & Archived",
    noPendingTasks = "No unfinished reminder tasks",
    noCompletedTasks = "No completed tasks or archived notes",
    markDone = "Mark Done",
    reminderDoneRepeated = "Reminder completed. Next: %s",
    reminderDoneOnce = "Reminder marked as done",
    reminderDoneMovedToHistory = "Task completed & moved to History & Archive",
    stopRecurring = "Stop Recurring",
    stopRecurringPermanently = "Stop Recurring Permanently",
    completeThisSession = "Complete This Session",
    completeThisSessionDesc = "Repeats again on %s",
    stopRecurringDesc = "Complete permanently and turn off reminder",
    recurringStopped = "Recurring reminder stopped",
    stopSchedule = "Stop",
    restore = "Restore",
    restoredToNotes = "Notes restored to home",
    completed = "Completed",
    overdue = "Overdue",
    cloudBackup = "Cloud Backup & Sync",
    cloudBackupDesc = "Sync notes to Supabase database",
    googleAccount = "Google Account",
    signInWithGoogle = "Sign in with Google",
    signOut = "Sign Out",
    syncNow = "Sync Now",
    syncing = "Syncing...",
    syncSuccess = "Successfully updated %d notes",
    syncUpToDate = "All notes are up to date",
    syncFailed = "Failed to sync with cloud",
    notLoggedIn = "Not signed in",
    supabaseNotConfigured = "Supabase credentials not configured in local.properties",
    lastSynced = "Last synced: %s",
    never = "Never",
    accountAndSync = "Account & Cloud Sync",
    preferences = "Preferences",
    aboutAndUpdates = "About & Updates",
    newBadge = "NEW",
    googleSignInPrompt = "Sign in with Google to securely back up & sync notes across your devices",
    signingIn = "Connecting...",
    autoSyncActive = "Auto-Sync Active",
    autoArchive = "Archive when completed",
    autoArchiveDesc = "Hide from home screen once reminder is marked done",
    alarmAutoArchiveNotice = "Auto-archives on dismiss",
    alarmKeepOnHomeNotice = "Keeps on home screen",
    archive = "Archive",
    notesArchived = "Notes archived"
)

val IndonesianStrings = AppStrings(
    locale = java.util.Locale("id", "ID"),
    back = "Kembali",
    save = "Simpan",
    cancel = "Batal",
    delete = "Hapus",
    markAsDone = "Tandai Selesai",
    untitled = "Tanpa Judul",
    edited = "Diedit",
    noNotesYet = "Belum ada catatan",
    pinned = "Disematkan",
    others = "Lainnya",
    newNote = "Catatan Baru",
    pin = "Sematkan",
    unpin = "Lepas Sematan",
    searchNotes = "Cari catatan...",
    scheduledAt = "Pengingat %s",
    editNote = "Edit Catatan",
    noteTitleHint = "Judul",
    noteContentHint = "Tulis catatan...",
    reminder = "Pengingat",
    priority = "Prioritas",
    notification = "Pengingat",
    alarm = "Alarm",
    setReminder = "Atur waktu & tanggal",
    clearReminder = "Hapus",
    selectDate = "Pilih Tanggal",
    selectTime = "Pilih Waktu",
    alarmIn = "Alarm dalam %s",
    reminderIn = "Pengingat dalam %s",
    lessThanOneMinute = "< 1 menit",
    timeHours = "jam",
    timeMinutes = "menit",
    timeDays = "hari",
    repeat = "Ulangi",
    repeatNone = "Tidak berulang",
    repeatDaily = "Setiap hari",
    repeatWeekly = "Setiap minggu",
    repeatMonthly = "Setiap bulan",
    repeatYearly = "Setiap tahun",
    selectedCount = "%d dipilih",
    selectAll = "Pilih Semua",
    deselectAll = "Batal Pilih Semua",
    noteColor = "Warna Catatan",
    colorDefault = "Standar",
    colorBlue = "Biru",
    colorRed = "Merah",
    colorYellow = "Kuning",
    colorGreen = "Hijau",
    colorPurple = "Ungu",
    colorRose = "Merah Rose",
    colorOrange = "Oranye",
    colorTeal = "Teal",
    checklist = "Kotak Centang",
    addChecklistItem = "Item daftar",
    checkedItemsCount = "%d item dicentang",
    settings = "Pengaturan",
    language = "Bahasa",
    english = "English",
    indonesian = "Bahasa Indonesia",
    theme = "Tema",
    themeSystem = "Sistem",
    themeLight = "Terang",
    themeDark = "Gelap",
    snoozeDuration = "Durasi Tunda",
    reminderSound = "Nada Pengingat",
    alarmSound = "Nada Alarm",
    defaultSound = "Default",
    minutes = "menit",
    exportBackup = "Ekspor Cadangan",
    exportBackupDesc = "Simpan catatan ke file JSON",
    importBackup = "Impor Cadangan",
    importBackupDesc = "Pulihkan catatan dari file JSON",
    exportSuccess = "Cadangan berhasil diekspor",
    exportFailed = "Gagal mengekspor cadangan",
    importSuccess = "Berhasil mengimpor %d catatan",
    importFailed = "Gagal mengimpor file cadangan",
    checkForUpdates = "Periksa Pembaruan",
    checkForUpdatesDesc = "Cek rilis terbaru di GitHub",
    alreadyLatest = "Aplikasi sudah versi terbaru",
    updateAvailable = "Pembaruan Tersedia",
    updateNow = "Perbarui Sekarang",
    later = "Nanti",
    downloadingUpdate = "Mengunduh pembaruan...",
    about = "Tentang",
    noteReminder = "PENGINGAT CATATAN",
    snooze = "Tunda",
    dismiss = "Matikan",
    history = "Riwayat",
    reminderHistory = "Riwayat & Arsip",
    pendingTasks = "Belum Selesai",
    completedTasks = "Selesai & Arsip",
    noPendingTasks = "Tidak ada tugas pengingat yang belum selesai",
    noCompletedTasks = "Belum ada tugas selesai atau catatan terarsip",
    markDone = "Tandai Selesai",
    reminderDoneRepeated = "Pengingat selesai. Jadwal berikutnya: %s",
    reminderDoneOnce = "Pengingat ditandai selesai",
    reminderDoneMovedToHistory = "Tugas selesai & dipindahkan ke Riwayat & Arsip",
    stopRecurring = "Hentikan Pengulangan",
    stopRecurringPermanently = "Hentikan Pengulangan Seterusnya",
    completeThisSession = "Selesai Sesi Ini",
    completeThisSessionDesc = "Diulang lagi pada %s",
    stopRecurringDesc = "Selesai permanen dan matikan jadwal",
    recurringStopped = "Pengulangan pengingat dihentikan",
    stopSchedule = "Hentikan",
    restore = "Pulihkan",
    restoredToNotes = "Catatan dikembalikan ke beranda",
    completed = "Selesai",
    overdue = "Terlewat",
    cloudBackup = "Cadangan & Sinkronisasi Cloud",
    cloudBackupDesc = "Sinkronkan catatan ke database Supabase",
    googleAccount = "Akun Google",
    signInWithGoogle = "Masuk dengan Google",
    signOut = "Keluar",
    syncNow = "Sinkronkan Sekarang",
    syncing = "Menyinkronkan...",
    syncSuccess = "Berhasil memperbarui %d catatan",
    syncUpToDate = "Semua catatan sudah sinkron",
    syncFailed = "Gagal menyinkronkan dengan cloud",
    notLoggedIn = "Belum masuk akun",
    supabaseNotConfigured = "Kredensial Supabase belum diatur di local.properties",
    lastSynced = "Terakhir sinkron: %s",
    never = "Belum pernah",
    accountAndSync = "Akun & Sinkronisasi Cloud",
    preferences = "Preferensi",
    aboutAndUpdates = "Tentang & Pembaruan",
    newBadge = "BARU",
    googleSignInPrompt = "Masuk dengan Google untuk mencadangkan & menyinkronkan catatan secara aman",
    signingIn = "Menghubungkan...",
    autoSyncActive = "Otomatis Tersinkron",
    autoArchive = "Arsipkan saat selesai",
    autoArchiveDesc = "Sembunyikan dari beranda setelah pengingat selesai",
    alarmAutoArchiveNotice = "Otomatis diarsipkan saat selesai",
    alarmKeepOnHomeNotice = "Tetap disimpan di beranda",
    archive = "Arsipkan",
    notesArchived = "Catatan diarsipkan"
)

val LocalStrings = staticCompositionLocalOf { EnglishStrings }
